...
# 		differentiate angle to get angular velocity
# 		TODO: once IMU-reported velocity is fixed, this won't be necessary
# 		self.angular_velocity = np.gradient(self.angle, T)
# 		low pass filter angular velocity
# 		self.angular_velocity = np.convolve(self.angular_velocity, np.ones(10) / 10, mode='same')

# 		correct odometry using new angular velocity,  because angular vel readings are broken
# 		basically redoes the job of OdometryR2.java and OdometryIntegrator
# 		TODO: once IMU-reported velocity is fixed, this won't be necessary
# 		self.x_velocity = self.x_velocity + 0.082767 * self.angular_velocity
# 		self.y_velocity = self.y_velocity + 0.075081 * self.angular_velocity
#
# 		global_x_vel = np.cos(self.angle) * self.x_velocity - np.sin(self.angle) * self.y_velocity
# 		global_y_vel = np.sin(self.angle) * self.x_velocity + np.cos(self.angle) * self.y_velocity
#
# 		self.x_position = np.cumsum(global_x_vel) * T
# 		self.y_position = np.cumsum(global_y_vel) * T
