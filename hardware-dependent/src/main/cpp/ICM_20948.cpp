...
ICM_20948_Status_e ICM_20948::checkID(void) {
    ...
    if (status != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::checkID: ICM_20948_check_id returned: %d", status);
    }
    return status;
}
...
bool ICM_20948::isConnected(void) {
    ...
    __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "ICM_20948::isConnected: checkID returned: %d",
                        status);
    return false;
}
...
ICM_20948_Status_e ICM_20948::setSampleMode(uint8_t sensor_id_bm, uint8_t lp_config_cycle_mode) {
    ...
    jsleep(1); // Give the ICM20948 time to change the sample mode (see issue #8)
    return status;
}
...
ICM_20948_Status_e ICM_20948::setSampleRate(uint8_t sensor_id_bm, ICM_20948_smplrt_t smplrt) {
    status = ICM_20948_set_sample_rate(&_device, (ICM_20948_InternalSensorID_bm) sensor_id_bm,
                                       smplrt);
    return status;
}
...
ICM_20948_Status_e ICM_20948::clearInterrupts(void) {
    ...
    status = ICM_20948_execute_r(&_device, AGB0_REG_INT_STATUS, (uint8_t *) &int_stat,
...
ICM_20948_Status_e
ICM_20948::i2cControllerConfigurePeripheral(uint8_t peripheral, uint8_t addr, uint8_t reg,
                                            uint8_t len, bool Rw,
                                            bool enable, bool data_only, bool grp, bool swap,
                                            uint8_t dataOut) {
    status = ICM_20948_i2c_controller_configure_peripheral(&_device, peripheral, addr, reg, len, Rw,
                                                           enable, data_only,
                                                           grp, swap, dataOut);
    return status;
}
...
ICM_20948_Status_e
ICM_20948::i2cMasterConfigureSlave(uint8_t peripheral, uint8_t addr, uint8_t reg, uint8_t len,
                                   bool Rw, bool enable,
                                   bool data_only, bool grp, bool swap) {
    return (i2cControllerConfigurePeripheral(peripheral, addr, reg, len, Rw, enable, data_only, grp,
                                             swap, 0));
}

ICM_20948_Status_e
ICM_20948::i2cControllerPeriph4Transaction(uint8_t addr, uint8_t reg, uint8_t *data, uint8_t len,
                                           bool Rw,
                                           bool send_reg_addr) {
    status = ICM_20948_i2c_controller_periph4_txn(&_device, addr, reg, data, len, Rw,
                                                  send_reg_addr);
    return status;
}
...
//https://www.oshwa.org/2020/06/29/a-resolution-to-redefine-spi-pin-names/
ICM_20948_Status_e
ICM_20948::i2cMasterSLV4Transaction(uint8_t addr, uint8_t reg, uint8_t *data, uint8_t len, bool Rw,
                                    bool send_reg_addr) {
...
uint8_t ICM_20948::i2cMasterSingleR(uint8_t addr, uint8_t reg) {
    ...
    if (status != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::i2cMasterSingleR: ICM_20948_i2c_master_single_r returned: %d",
                            status);
    }
    return data;
}
...
ICM_20948_Status_e ICM_20948::startupDefault(bool minimal) {
    ...
    if (retval != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupDefault: checkID returned: %d", retval);
        status = retval;
        return status;
    }
    ...
    if (retval != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupDefault: swReset returned: %d", retval);
        status = retval;
        return status;
    }
    jsleep(50);
    ...
    if (retval != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupDefault: sleep returned: %d", retval);
        status = retval;
        return status;
    }
    ...
    if (retval != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupDefault: lowPower returned: %d", retval);
        status = retval;
        return status;
    }
    ...
    if (retval != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupDefault: startupMagnetometer returned: %d", retval);
        status = retval;
        return status;
    }
    ...
    if (retval != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupDefault: setSampleMode returned: %d", retval);
        status = retval;
        return status;
    ...
    if (retval != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupDefault: setFullScale returned: %d", retval);
        status = retval;
        return status;
    }
    ...
    if (retval != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupDefault: setDLPFcfg returned: %d", retval);
        status = retval;
        return status;
    }
    ...
    if (retval != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupDefault: enableDLPF (Acc) returned: %d", retval);
        status = retval;
        return status;
    }
    ...
    if (retval != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupDefault: enableDLPF (Gyr) returned: %d", retval);
        status = retval;
        return status;
    }
...
ICM_20948_Status_e ICM_20948::enableDMPSensor(enum inv_icm20948_sensor sensor, bool enable) {
    if (_device._dmp_firmware_available == true) // Should we attempt to enable the sensor?
    {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::enableDMPSensor:  _enabled_Android_0: %d  _enabled_Android_1: %d  _dataOutCtl1: %d  _dataOutCtl2: %d  _dataRdyStatus: %d",
                            _device._enabled_Android_0, _device._enabled_Android_1,
                            _device._dataOutCtl1, _device._dataOutCtl2, _device._dataRdyStatus);
        return status;
    }
    return ICM_20948_Stat_DMPNotSupported;
}
...
ICM_20948_Status_e ICM_20948::enableDMPSensorInt(enum inv_icm20948_sensor sensor, bool enable) {
        ...
        true) // Should we attempt to enable the sensor interrupt?
    {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::enableDMPSensorInt:  _enabled_Android_intr_0: %d  _enabled_Android_intr_1: %d  _dataIntrCtl: %d",
                            _device._enabled_Android_intr_0, _device._enabled_Android_intr_1,
                            _device._dataIntrCtl);
        return status;
    }
    return ICM_20948_Stat_DMPNotSupported;
}
...
ICM_20948_Status_e
ICM_20948::writeDMPmems(unsigned short reg, unsigned int length, const unsigned char *data) {
...
ICM_20948_Status_e
ICM_20948::readDMPmems(unsigned short reg, unsigned int length, unsigned char *data) {
...
ICM_20948_Status_e ICM_20948::setGyroSF(unsigned char div, int gyro_level) {
    if (_device._dmp_firmware_available == true) // Should we attempt to set the Gyro SF?
    {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::setGyroSF:  pll: %d,  Gyro SF is: %d", _device._gyroSFpll,
                            _device._gyroSF);
        return status;
    }
    return ICM_20948_Stat_DMPNotSupported;
...
ICM_20948_Status_e ICM_20948::initializeDMP(void) {
    ...
    if (_device._dmp_firmware_available != true) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupDMP: DMP is not available. Please check that you have uncommented line 29 (#define ICM_20948_USE_DMP) in ICM_20948_C.h...");
        return ICM_20948_Stat_DMPNotSupported;
    }
    ...
    result = i2cControllerConfigurePeripheral(0, MAG_AK09916_I2C_ADDR, AK09916_REG_RSV2, 10, true,
    ...
    result = i2cControllerConfigurePeripheral(1, MAG_AK09916_I2C_ADDR, AK09916_REG_CNTL2, 1, false,
                                              true, false, false, false, AK09916_mode_single);
    if (result > worstResult) worstResult = result;
...
ICM_20948::ICM_20948(
        ...
        void *user
) : jsleep{jsleep}, status{ICM_20948_init_struct(&_device)}, _serif{
        .write = write,
        .read = read,
        .user = user
} {
    ...
    if (status != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::begin: startupDefault returned: %d", status);
    }
}
...
ICM_20948_Status_e ICM_20948::startupMagnetometer(bool minimal) {
    ...
    if (tries == MAX_MAGNETOMETER_STARTS) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupMagnetometer: reached MAX_MAGNETOMETER_STARTS (%d). Returning ICM_20948_Stat_WrongID",
...
//        else
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupMagnetometer: successful magWhoIAm after %d tries",
                            tries);
    }
    ...
    if (retval != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupMagnetometer: writeMag returned: %d", retval);
        status = retval;
        return status;
    }

    retval = i2cControllerConfigurePeripheral(0, MAG_AK09916_I2C_ADDR, AK09916_REG_ST1, 9, true,
                                              true, false, false,
                                              false);
    if (retval != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::startupMagnetometer: i2cMasterConfigurePeripheral returned: %d",
                            retval);
        status = retval;
        return status;
    }
...
ICM_20948_Status_e ICM_20948::magWhoIAm(void) {
    ...
    if (retval != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::magWhoIAm: whoiam1: %d (should be 72) readMag set status to: %d",
                            whoiam1, status);
        return retval;
    }
    ...
    if (retval != ICM_20948_Stat_Ok) {
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                            "ICM_20948::magWhoIAm: whoiam1: %d (should be 72) whoiam2: %d (should be 9) readMag set status to: %d",
                            whoiam1, whoiam2, status);
        return retval;
    }
    ...
    __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG,
                        "ICM_20948::magWhoIAm: whoiam1: %d (should be 72) whoiam2: %d (should be 9). Returning ICM_20948_Stat_WrongID",
                        whoiam1, whoiam2);
