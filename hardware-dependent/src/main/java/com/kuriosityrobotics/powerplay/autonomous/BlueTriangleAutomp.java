...
public class BlueTriangleAutomp extends OrchestratedOpMode {
	...
	protected void initOpMode(HardwareOrchestrator or) {
		...
		or.startNode("cam", new LeftCamera(or));
	...
	public void runOpMode(HardwareOrchestrator or) throws InterruptedException, ExecutionException {
		...
		Pose startPoint = new Pose(0.17, 1.0, toRadians(-90));
		Point preloadPoint = new Point(1.1 + .59, 0.92);
		...
		var cyclePoint = preloadPoint.add(new Point(-.35, .03));
...
//		or.dispatchSynchronous("mpc/motorWeight", .05);
		...
		var cycleToP1 = new Path(cyclePoint, Point.ofTileNumber(1, 1), Point.ofTileNumber(1, 2).add(new Point(-.04, 0))); // subtrsct 2cm so the intake doesnt overhang
		var cycleToP2 = new Path(cyclePoint, Point.ofTileNumber(1, 1).add(new Point(-.04, 0)));
		var cycleToP3 = new Path(cyclePoint, Point.ofTileNumber(1, 1), Point.ofTileNumber(1, 0).add(new Point(-.04, 0)));
